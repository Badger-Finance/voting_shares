...
def test_badger_sett_balance(prep_mint_bBadger, badger_voter, random_minter, badger):
    ...
    balance = badger_voter.badgerBalanceOf(random_minter) - badger.balanceOf(
        random_minter
    )
...
def test_balancer_mint(prep_mint_balancer, badger_voter, random_minter):
    # 3.5977908... is the current value of 1 BPT in badger
    # We will need to update this to be dynamic once the guestlist is removed
    amount_deposited = 3.59779081426
    balance = badger_voter.balancerBalanceOf(random_minter)
    print(f"received balance of {balance}")
    balance_normalized = balance / 1e18

    print(f"balancer balance: {balance_normalized}")
