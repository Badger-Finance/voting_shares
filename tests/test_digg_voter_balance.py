...
def test_sushi_voter_balance(
    prep_mint_sushi, digg_voter, random_digg_depositor, digg_wBTC_SLP
):
    ...
    balance = digg_voter.sushiswapBalanceOf(random_digg_depositor)
    balance_normalized = balance / 1e9
...
def test_uni_voter_balance(
    prep_mint_uni, digg_voter, random_digg_depositor, digg_wBTC_UniV2
):
    amount_deposited = 1
...
def test_digg_sett_balance(prep_mint_bDigg, digg_voter, random_digg_depositor, digg):
    amount_deposited = 1
    balance = digg_voter.diggBalanceOf(random_digg_depositor) - digg.balanceOf(
