...
@pytest.mark.require_network("mainnet-fork")
@pytest.fixture(scope="module")
def bpt_whale(accounts):
    yield accounts.at("0xBCff81420d024627eDd07AB9468Aff7881805D57", force=True)


@pytest.mark.require_network("mainnet-fork")
@pytest.fixture(scope="module")
def eth_whale(accounts):
    yield accounts.at("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2", force=True)
...
@pytest.mark.require_network("mainnet-fork")
@pytest.fixture(scope="module")
def badger_wbtc_balancer_token(interface):
    yield interface.IBalancerPoolToken("0xb460DAa847c45f1C4a41cb05BFB3b51c92e41B36")


@pytest.mark.require_network("mainnet-fork")
@pytest.fixture(scope="module")
def badger_wbtc_balancer_vault(interface):
    yield interface.ISett("0x63ad745506BD6a3E57F764409A47ed004BEc40b1")
...
def univ2_deposit(
    ...
    test_amount_token0 = int(
        (test_amount_token1 / token1_decimals)
        * (reserve0_normalized / reserve1_normalized)
        * token0_decimals
    )

    print(
        f"Depositing {test_amount_token0} token0 \
            and {test_amount_token1} token1"
    )
    ...
    print(
        f"Transferred: {test_amount_token1} \
        token1 {token1.balanceOf(depositor)}"
    )
    ...
    token1.approve(
        router,
        test_amount_token1,
        {"from": depositor, "gas_price": ExponentialScalingStrategy(7, 200)},
    )
...
def prep_mint_bDigg(digg_whale, random_digg_depositor, digg, digg_sett):
    ...
    digg.transfer(
        random_digg_depositor,
        test_amount,
        {"from": digg_whale, "gas_price": ExponentialScalingStrategy(7, 200)},
    )
...
def prep_mint_sushi(
    ...
    univ2_deposit(
        ...
        sushi_router,
        random_digg_depositor,
        {digg: digg_whale, wBTC: wbtc_whale},
    )
    print(
        f"Deposited to sushi, got {digg_wBTC_SLP.balanceOf(random_digg_depositor)} SLP"
    )
...
def prep_mint_uni(
    ...
    print(
        f"Deposited to uni, got \
         {digg_wBTC_UniV2.balanceOf(random_digg_depositor)} UniV2"
    )


@pytest.fixture(scope="module")
def digg_voter(DiggVotingShare, random_digg_depositor):
    diggVotingShare = DiggVotingShare.deploy(
        {
            "from": random_digg_depositor,
            "gas_price": ExponentialScalingStrategy(7, 200),
        }
...
def prep_mint_bBadger(badger_whale, random_minter, badger, badger_sett):
    ...
    badger.transfer(
        random_minter,
        test_amount,
        {"from": badger_whale, "gas_price": ExponentialScalingStrategy(7, 200)},
    )
...
def badger_voter(BadgerVotingShare, random_minter):
    badgerVotingShare = BadgerVotingShare.deploy(
        {"from": random_minter, "gas_price": ExponentialScalingStrategy(7, 200)}
    )
...
def arb_badger_voter(ArbBadgerVotingShare, random_minter):
    badgerVotingShare = ArbBadgerVotingShare.deploy(
        {"from": random_minter, "gas_price": ExponentialScalingStrategy(7, 200)}
    )
    yield badgerVotingShare
...
def prep_mint_curve(
    ...
    # We want to know how much token0 to deposit to maintain token1 levels
    test_amount_wbtc = int(
        Decimal(test_amount_badger / badger_decimals)
        * (wbtc_normalized / badger_normalized)
        * wbtc_decimals
    )
    ...
    wBTC.transfer(
        random_minter,
        test_amount_wbtc,
        {"from": wbtc_whale, "gas_price": ExponentialScalingStrategy(7, 200)},
    )
    print(f"Transferred: {test_amount_wbtc} token0 {wBTC.balanceOf(random_minter)}")

    badger.approve(
        badger_wBTC_crv_pool,
        int(test_amount_badger),
        {
            "from": random_minter,
            "gas_price": ExponentialScalingStrategy(7, 200),
        },
    ...
    badger_wBTC_crv_pool.add_liquidity(
        [int(test_amount_badger), test_amount_wbtc],
        deadline_unix,
        {
            "from": random_minter,
            "gas_price": ExponentialScalingStrategy(7, 200),
...
@pytest.fixture(scope="module")
def prep_mint_balancer(
    bpt_whale,
    random_minter,
    badger_wbtc_balancer_vault,
):
    test_amount = 1 * 1e18
    badger_wbtc_balancer_vault.transfer(
        random_minter,
        test_amount,
        {"from": bpt_whale, "gas_price": ExponentialScalingStrategy(7, 200)},
    )

    print(
        f"Deposited in the badger balancer pool: \
          {badger_wbtc_balancer_vault.balanceOf(random_minter)}"
    )
